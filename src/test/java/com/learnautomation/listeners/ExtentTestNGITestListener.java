...
public class ExtentTestNGITestListener implements ITestListener {
	...
	public synchronized void onTestFailure(ITestResult result) {
		...
		//WebDriver driver1 =(WebDriver)result.getTestClass().getRealClass().getDeclaredField("driver").get(result.getInstance());
		try {
			Field f=result.getTestClass().getRealClass().getDeclaredField("driver");
			Object obj =f.get(result.getInstance());
			driver=(WebDriver)obj;
			System.out.println("Driver value using reflection is "+driver);
		} catch (Exception e1) {
		...
		try {
			parentTest.get().fail("Test Failed" + result.getThrowable().getMessage(), MediaEntityBuilder
					.createScreenCaptureFromBase64String(Utility.captureScreenshotBase64(driver)).build());
